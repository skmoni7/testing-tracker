...
fun AddEditScreen(orderId: String, onSaved: () -> Unit, onCancel: () -> Unit) {
    ...
    Column(
        modifier = Modifier.fillMaxSize().background(bgColor).verticalScroll(rememberScrollState()).padding(24.dp)
    ) {
        Text(if (isNew) "Add Order" else "Edit Order", fontSize = 24.sp, fontWeight = FontWeight.Bold, color = textColor)
        Spacer(Modifier.height(24.dp))

        FormLabel("Product Name *", mutedColor)
        OutlinedTextField(value = productName, onValueChange = { productName = it }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("Product name", color = mutedColor) }, colors = fieldColors)
        Spacer(Modifier.height(12.dp))

        FormLabel("Order Number", mutedColor)
        OutlinedTextField(value = orderNumber, onValueChange = { orderNumber = it }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("Order #", color = mutedColor) }, colors = fieldColors)
        Spacer(Modifier.height(12.dp))

        FormLabel("Marketplace", mutedColor)
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp), modifier = Modifier.fillMaxWidth()) {
            MARKETPLACES.forEach { m -> FilterChip(selected = marketplace == m, onClick = { marketplace = m }, label = { Text(m, fontSize = 12.sp) }) }
        }
        Spacer(Modifier.height(12.dp))

        FormLabel("Seller Name", mutedColor)
        OutlinedTextField(value = sellerName, onValueChange = { sellerName = it }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("Seller name", color = mutedColor) }, colors = fieldColors)
        Spacer(Modifier.height(12.dp))

        FormLabel("Product Price ($)", mutedColor)
        OutlinedTextField(value = price, onValueChange = { price = it }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("0.00", color = mutedColor) }, colors = fieldColors)
        Spacer(Modifier.height(12.dp))

        FormLabel("Commission Amount ($)", mutedColor)
        OutlinedTextField(value = commissionAmount, onValueChange = { commissionAmount = it }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("0.00", color = mutedColor) }, colors = fieldColors)
        Spacer(Modifier.height(12.dp))

        FormLabel("PayPal Account", mutedColor)
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            PAYPAL_ACCOUNTS.forEach { pp -> FilterChip(selected = paypalAccount == pp, onClick = { paypalAccount = pp }, label = { Text(pp, fontSize = 12.sp) }) }
        }
        Spacer(Modifier.height(12.dp))

        FormLabel("Review Type", mutedColor)
        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            REVIEW_TYPES.forEach { rt -> FilterChip(selected = reviewType == rt, onClick = { reviewType = rt }, label = { Text(rt, fontSize = 12.sp) }) }
        }
        ...
        Button(
            onClick = {
                ...
                if (isNew) {
                    data["delivered"] = false; data["reviewWritten"] = false; data["paymentReceived"] = false
                    data["amountCredited"] = 0.0
                    data["deliveredAt"] = Timestamp(0,0); data["reviewWrittenAt"] = Timestamp(0,0); data["paymentReceivedAt"] = Timestamp(0,0)
                    data["createdAt"] = Timestamp.now()
                    db.collection("orders").add(data).addOnSuccessListener { onSaved() }.addOnFailureListener { e -> errorMsg = e.message ?: "Error"; loading = false }
                } else {
                    db.collection("orders").document(orderId).update(data).addOnSuccessListener { onSaved() }.addOnFailureListener { e -> errorMsg = e.message ?: "Error"; loading = false }
                }
            },
        ...
        OutlinedButton(onClick = onCancel, modifier = Modifier.fillMaxWidth().height(50.dp), shape = RoundedCornerShape(8.dp)) {
