...
function getRowBg(order: Order): string {
  if (order.paymentReceived) return 'bg-green-100 dark:bg-green-900 border-l-4 border-green-500';
  if (order.delivered && order.reviewWritten) return 'bg-yellow-50 dark:bg-yellow-900/40 border-l-4 border-yellow-500';
  if (order.delivered) return 'bg-orange-50 dark:bg-orange-900/30 border-l-4 border-orange-500';
...
export default function DashboardPage() {
  ...
  const receivedAmount = orders.filter(o => o.paymentReceived).reduce((sum, o) => sum + (o.price || 0) + (o.commissionAmount || 0), 0);
  ...
  // ----- Colored status checkbox box -----
  const StatusBox = ({
    label, checked, ts, field, orderId
  }: {
    label: string; checked: boolean; ts: Timestamp | null | undefined;
    field: 'delivered' | 'reviewWritten' | 'paymentReceived'; orderId: string;
  }) => {
    ...
    return (
      ...
      >
        ...
        <input
          ...
          className={`w-4 h-4 cursor-pointer my-1 ${checked ? 'accent-green-600' : 'accent-red-500'}`}
        />
  ...
  // ----- Amt Cr styled box: label top, input middle, Save button bottom -----
  const AmtCrBox = ({ order, total }: { order: Order; total: number }) => {
    const val = getAmtCrValue(order);
    const numVal = parseFloat(val) || 0;
    const colorClass = numVal <= 0
      ? 'text-gray-400'
      : numVal >= total
        ? 'text-green-600 dark:text-green-400'
        : 'text-red-500 dark:text-red-400';
    return (
      <div className="flex flex-col items-center rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-2 py-1.5 w-[80px] min-h-[72px] justify-between gap-1">
        {/* Label top */}
        <span className="text-[9px] text-gray-400 uppercase tracking-wide font-semibold">Amt Cr</span>
        {/* Input middle */}
        <input
          type="number"
          step="0.01"
          ...
          onKeyDown={e => { if (e.key === 'Enter') saveAmtCr(order.id, val); }}
          className={`w-full text-center text-sm font-bold bg-transparent focus:outline-none ${colorClass}`}
        />
        {/* Save button bottom */}
        <button
          onClick={() => saveAmtCr(order.id, val)}
          className="text-[9px] text-teal-500 hover:text-teal-400 font-semibold leading-tight"
        >
          Save
        </button>
      </div>
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 text-gray-900 dark:text-gray-100">
      ...
      <nav className="bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 px-4 py-2 flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => router.push('/add')} className="bg-indigo-600 text-white text-sm font-semibold px-3 py-1.5 rounded-lg hover:bg-indigo-700 transition">+ Add</button>
      ...
      {!loading && orders.length === 0 && <p className="text-center mt-10 text-gray-500">No records yet. Click + Add to start.</p>}
                ...
                return (
                  <tr key={order.id} className={`border-b border-gray-200 dark:border-gray-700 ${getRowBg(order)}`}>
                    <td className="px-3 py-2">
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full ${getBadge(order)}`}>{getLabel(order)}</span>
                    </td>
                    <td className="px-3 py-2">
                    ...
                    <td className="px-3 py-2">
                      ...
                      <div className="text-[10px] text-gray-400">{order.reviewType === 'text+pic' ? 'Txt+Pic' : 'Text'}</div>
                    </td>
                    <td className="px-3 py-2">{order.paypalAccount}</td>
                    <td className="px-3 py-2 text-right">
                      <div>${order.price.toFixed(2)}</div>
                      {commission > 0 && <div className="text-[10px] text-gray-400">+${commission.toFixed(2)} comm</div>}
                    </td>
                    <td className="px-3 py-2 text-right font-bold">${total.toFixed(2)}</td>
                    {/* Amt Cr box */}
                    <td className="px-2 py-2 text-center">
                      <AmtCrBox order={order} total={total} />
                    </td>
                    <td className="px-2 py-2 text-center">
                    ...
                    <td className="px-3 py-2 text-center">
                      <div className="flex gap-3">
                        <button onClick={() => router.push(`/edit/${order.id}`)} className="text-indigo-600 hover:underline text-sm font-medium">Edit</button>
                        <button onClick={() => deleteOrder(order.id)} className="text-red-500 hover:underline text-sm">Del</button>
                      </div>
                    </td>
                  </tr>
                );
            ...
            return (
              <div key={order.id} className={`rounded-xl p-3 shadow-sm ${getCardBorder(order)}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        {order.sellerName && <div className="text-xs text-gray-500">{order.sellerName}</div>}
                      </div>
                      <span className={`text-xs font-semibold px-2 py-0.5 rounded-full whitespace-nowrap ${getBadge(order)}`}>{getLabel(order)}</span>
                    </div>
                ...
                <div className="flex gap-3 mt-2">
                  <button onClick={() => router.push(`/edit/${order.id}`)} className="text-indigo-600 hover:underline text-sm font-medium">Edit</button>
                  <button onClick={() => deleteOrder(order.id)} className="text-red-500 hover:underline text-sm">Delete</button>
                </div>
              </div>
            );
          })}
